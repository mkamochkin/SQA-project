...
class Transactions:
    ...
    def withdraw(self):
        if self.isAdmin:
            ...
            if self.status != "A":
                print("This account is not active. Transaction denied.")
    ...
    def transfer(self):
        ...
        if self.isAdmin:
            ...
            if src_status != "A":
                print("Source account is not active. Transaction denied.")
                getTransactionInput(self.isAdmin)
                return
        ...
        if dest_status != "A":
            print("Destination account is not active. Transaction denied.")
            getTransactionInput(self.isAdmin)
            return
    ...
    def delete(self):
        # Privileged transaction: only allowed in admin mode.
        if not self.isAdmin:
        ...
        # Retrieve the account record by name.
        record_line = self.getRecordLineFromName(name)
        if record_line is None:
            print("Account not found for name:", name)
            self.delete()  # Retry
            return

        # Verify that the provided account number matches the record.
        acct_num_in_record = record_line[0:5]
        if acct_num_in_record != str(number).zfill(5):
            print("Account number does not match the account holder's record. Try again.")
            self.delete()  # Retry
            return

        # Check if the account is already deleted.
        current_status = record_line[27]
        if current_status == "X":
            print("Account is already deleted. No further transactions will be accepted for this account.")
            getTransactionInput(self.isAdmin)
            return

        # Use regex to update the status from "A" to "X" in the record.
        # The CBA record format is: NNNNN_AAAAAAAAAAAAAAAAAAAA_S_PPPPPPPP
        # We allow any 20 characters for the name field.
        import re
        pattern = r"^(\d{5}_.{20}_)[A](_.{8})$"
        new_record = re.sub(pattern, r"\1X\2", record_line)
        if new_record == record_line:
            print("Failed to update account status. Please try again.")
            self.delete()
            return

        # Record the delete transaction in BAT using transaction code 06.
        BATString = BAT_Serializer.serialize(6, name, acct_num_in_record, "0", "XX")
        with open(self.temp_bat_file, "a") as temp_bat:
            temp_bat.write(BATString)

        # Update the account record in the permanent CBA file (or via your CBA_Writer)
        CBA_Writer.writeToCBA(new_record)

        print("Account " + acct_num_in_record + " (" + name + ") has been deleted.")
        print("No further transactions will be accepted on this account.")
        getTransactionInput(self.isAdmin)
    ...
    def disable(self):
        ...
        # Verify the provided account number matches the record.
        acct_num_in_record = record_line[0:5]
        if acct_num_in_record != str(number).zfill(5):
            print("Account number does not match the account holder's record. Try again.")
            self.disable()  # Retry
        ...
        # The status character is at position 27 (based on fixed-width format).
        current_status = record_line[27]
        if current_status == "D":
            print("Account is already disabled. No further transactions will be accepted for this account.")
            getTransactionInput(self.isAdmin)
            return

        # Use regex to replace the status from "A" to "D" in the record line.
        # The CBA record format: NNNNN_AAAAAAAAAAAAAAAAAAAA_S_PPPPPPPP
        # Pattern breakdown:
        #   - ^(\d{5}_[^_]{20}_) : 5 digits, underscore, 20 characters for name, underscore.
        #   - [A]                : the status letter "A".
        #   - (_\d{8})$         : underscore and 8 characters for the balance.
        
        pattern = r"^(\d{5}_.{20}_)[A](_.{8})$"

        new_record = re.sub(pattern, r"\1D\2", record_line)
        if new_record == record_line:
            print("Failed to update account status. Please try again.")
            self.disable()
            return

        # Record the disable transaction in BAT using transaction code 07.
        BATString = BAT_Serializer.serialize(7, name, acct_num_in_record, "0", "XX")
        with open(self.temp_bat_file, "a") as temp_bat:
            temp_bat.write(BATString)

        # Update the account record in the permanent CBA file.
        # (CBA_Writer.writeToCBA is assumed to update the record accordingly.)
        CBA_Writer.writeToCBA(new_record)

        print("Account " + acct_num_in_record + " (" + name + ") has been disabled.")
        getTransactionInput(self.isAdmin)
...
def checkIfValidUser(name):
        """
        Check if the account holder's name exists in the permanent CBA file.
        ...
        try:
            with open("CBA.txt", "r") as file:
                for line in file:
                    # The account holder's name is stored in positions 6 to 26.
                    acct_name = line[6:26].replace("_", " ").strip()
                    if acct_name.lower() == name.lower():
                        return True
        except FileNotFoundError:
            print("Permanent CBA file not found.")
        return False

def checkIfActive(name):
    """
    Check if the account holder's account is disabled.
    Looks up the record in the permanent CBA file and returns True if the status is "D".
    """
    try:
        with open("CBA.txt", "r") as file:
            for line in file:
                acct_name = line[6:26].replace("_", " ").strip()
                if acct_name.lower() == name.lower():
                    # The status character is at position 27 in the fixed-width record.
                    if line[27] == "A":
                        return True
                    else:
                        return False
